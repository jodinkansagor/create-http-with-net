...
const app = net.createServer(socket => {
  socket.on('data', data => {
    ...
    if (request.method === 'GET' && request.path === '/') {
      ...
      socket.end(createResponse({ body: '<h1 style=color:red>red</h1>', status: '200 OK', contentType: 'text/html' }));
    } else if(request.method === 'GET' && request.path === '/green') {
      socket.end(createResponse({ body: '<h1 style=color:green>green</h1>', status: '200 OK', contentType: 'text/html' }));
    } else if (request.method === 'GET' && request.path === '/blue') {
      socket.end(createResponse({ body: '<h1 style=color:blue>blue</h1>', status: '200 OK', contentType: 'text/html' }));
